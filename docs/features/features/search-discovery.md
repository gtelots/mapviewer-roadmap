...
| Search & Discovery | Autocomplete Suggestions | Gợi ý tự động khi nhập | Hiển thị top 5 kết quả phù hợp nhất khi người dùng gõ ký tự đầu tiên | Tăng tốc độ tìm kiếm, chuẩn UX của Google Maps và Apple Maps | Phase 1 |
...
| Search & Discovery | Reverse Geocoding | Chuyển tọa độ thành địa chỉ | Click vào bản đồ để lấy địa chỉ gần nhất, hỗ trợ offline | Essential cho share location và emergency services | Phase 1 |
| Search & Discovery | Coordinate Search | Tìm kiếm bằng tọa độ | Nhập lat,lng hoặc MGRS để zoom đến vị trí chính xác | Critical cho professional users (GIS, surveying, military) | Phase 1 |
